...
export async function fetchThreadVariables(threadId: string): Promise<Record<string, any>> {
  const res = await api.get(`/threads/${threadId}/variables`);
...
export async function updateThreadVariable(params: {
  ...
  await api.post(`/threads/${params.threadId}/variables/${encodeURIComponent(params.variableName)}`, params.value);
}
