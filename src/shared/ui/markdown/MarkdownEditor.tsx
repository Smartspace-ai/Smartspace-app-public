...
export type FetchMentionUsers = (
  workspaceId: string
) => Promise<MentionUserLike[]>;
...
function EditorInner({
  ...
  async function insertUploadedFilesIntoEditor(files: File[]) {
    ...
    for (const file of files) {
      ...
      if (isImage) {
        ...
        try {
          const type = getSchemaNodeType(view, 'ssImage');
          if (type) {
        ...
        try {
          const type = getSchemaNodeType(view, 'fileTag');
          if (type) {
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      getPlainText: () => {
        ...
        const leafText = (node: PMNode) => {
          try {
            if (node?.type?.name === 'mention') {
              ...
              return `@${displayName}${mentionBoundary}`;
            }
          } catch {
            /* ignore */
          }
        ...
        if (!rawText.includes(mentionBoundary)) return rawText.trimEnd();
        const chars = [...rawText];
        for (let i = 0; i < chars.length; i += 1) {
          if (chars[i] !== mentionBoundary) continue;
  ...
  function insertMention(user: MentionUserLike) {
    try {
      ...
      const nextChar = view.state.doc.textBetween(from, from + 1, '\n', '\n');
      if (!/[\s]/.test(nextChar)) {
        insertTextAtSelection(view, ' ');
      }
      setMentionOpen(false);
