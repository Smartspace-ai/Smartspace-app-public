...
describe('request/unwrap', () => {
  it('returns ok result on success', async () => {
    const spy = vi
      .spyOn(apiClient.api, 'request')
      .mockResolvedValueOnce({ data: { a: 1 } } as unknown as {
        data: { a: number };
      });
  ...
  it('requestOrThrow throws on Err and returns data on Ok', async () => {
    const success = vi
      .spyOn(apiClient.api, 'request')
      .mockResolvedValueOnce({ data: { ok: true } } as unknown as {
        data: { ok: boolean };
      });
    await expect(
      requestOrThrow({ method: 'GET', url: '/ok' })
    ...
    await expect(
      requestOrThrow({ method: 'GET', url: '/fail' })
  ...
  it('unwrap returns data or throws', () => {
    ...
    expect(() =>
      unwrap({ ok: false, error: { type: 'Unauthorized' } })
    ).toThrow();
  });
});
