...
describe('toAppError', () => {
  it('maps known status codes', () => {
    ...
    expect(toAppError(409, { message: 'x' })).toEqual({
      type: 'Conflict',
      message: 'x',
    });
    expect(toAppError(422, { issues: ['e'] })).toEqual({
      type: 'ValidationError',
      issues: ['e'],
    });
    expect(toAppError(429, { retryAfter: 5 })).toEqual({
      type: 'RateLimited',
      retryAfterSec: 5,
    });
  });
  ...
  it('maps unknown codes to UnknownError', () => {
    expect(toAppError(500, { message: 'boom' })).toEqual({
      type: 'UnknownError',
      status: 500,
      message: 'boom',
    });
