...
describe('apiParsed/getParsed/requestParsed', () => {
  ...
  it('requestParsed uses requestOrThrow and parses', async () => {
    ...
    const spy = vi
      .spyOn(requestMod, 'requestOrThrow')
      .mockResolvedValueOnce({ b: 'ok' } as unknown as { b: string });
    await expect(
      requestParsed(schema, { method: 'POST', url: '/y' })
    ).resolves.toEqual({ b: 'ok' });
    spy.mockResolvedValueOnce({ b: 1 } as unknown as { b: string });
    await expect(
      requestParsed(schema, { method: 'PUT', url: '/y' })
  ...
  it('apiParsed sugar methods call underlying functions', async () => {
    ...
    const gp = vi
      .spyOn(apiMod.api, 'get')
      .mockResolvedValueOnce({ c: 'c' } as unknown as { c: 'c' });
