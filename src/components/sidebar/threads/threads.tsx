...
export function Threads() {  
  ...
  useEffect(() => {
    // Reset auto-generation flag when workspace changes
    if (hasAutoGeneratedRef.current !== urlWorkspaceId) {
      hasAutoGeneratedRef.current = null;
    }

    // Auto-generate thread only if no threads exist and we haven't already generated one for this workspace
    if (!isLoading && !threadId && (threads?.length ?? 0) === 0 && hasAutoGeneratedRef.current !== urlWorkspaceId) {
      const newThreadId = crypto.randomUUID();
      setAutoCreatedThreadId(newThreadId);
      hasAutoGeneratedRef.current = urlWorkspaceId || null;
      
      if (urlWorkspaceId) {
        // Set the query cache to 404 for the auto-created thread
        queryClient.setQueryData(['workspace', urlWorkspaceId, 'thread', newThreadId], null);
        queryClient.setQueryData(['comments', newThreadId], null);
        queryClient.setQueryData(['thread', newThreadId, 'variables'], null);
    ...
    // Replace auto-generated thread with real thread when threads are loaded
    if (threads && (threads.length ?? 0) > 0 && autoCreatedThreadId && threadId === autoCreatedThreadId) {
