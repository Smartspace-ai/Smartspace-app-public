...
const tunnelRequired =
  String(process.env.TUNNEL_REQUIRED || '').toLowerCase() === 'true';
...
try {
  ...
  console.log(
    `Starting dev tunnel for :${port} (expected public origin host: ${host})`
  );
  ...
  console.warn(
    `Invalid PUBLIC_ORIGIN ("${origin}"). Skipping dev tunnel.`,
    err?.message ?? err
  );
  process.exit(0);
}
