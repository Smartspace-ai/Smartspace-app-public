...
type JsonSchemaWithProps = JsonSchema & {
  ...
  if (isType(s.type, 'object') && s.properties) {
    ...
    for (const [k, prop] of Object.entries(s.properties)) {
      if (hasDefault(prop)) defaults[k] = prop.default;
        ...
        isType(prop.type, 'object') &&
        (prop as JsonSchemaWithProps).properties
      )
        defaults[k] = getDefaultValues(prop) ?? {};
      else if (
        isType(prop.type, 'array') &&
        (prop as { items?: unknown }).items
      )
        defaults[k] = [];
